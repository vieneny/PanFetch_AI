...
- LLM 只生成 JSON 下载计划；每次计划保存在本地历史列表中，完整详情在后台读取，并使用按展开懒加载的文件夹优先目录树展示文件数量、总大小、排除原因和目标路径。
...
下载计划列表                         下载计划详情
┌──────────────────────────────┐    ┌──────────────────────────────────┐
│ 时间 / 需求 / 来源 / 数量    │ -> │ 文件夹树 / 规则 / 保存位置       │
└──────────────────────────────┘    └──────────────────────────────────┘

下载计划详情
┌────────────────────────────────────────────────────────────────────┐
│ 来源与筛选规则 / 文件夹优先目录树 / 保存位置 / 确认下载           │
└────────────────────────────────────────────────────────────────────┘
...
当前“识别资料内容”基于网盘元数据和目录结构，不会在未确认的情况下下载并解析文档正文。对话历史保存在 `.panfetch-ai/assistant_history.jsonl`；新生成的下载计划保存在 `.panfetch-ai/download_plans.db`，包含需求、规则和候选路径。两者都只存在本机并被 Git 忽略。计划列表只查询轻量摘要；点击“查看详情”后，完整 JSON 在 Qt 工作线程中恢复。详情树初始只创建顶层目录，展开时才创建直属子目录和文件；勾选状态由完整文件模型维护，因此未展开目录也能正确全选、反选和下载。

目录索引按已授权账号的 UID 隔离。切换百度账号后，搜索、目录缓存和统计只读取当前账号的数据，不会与旧账号的同名路径或文件 ID 冲突。升级自早期单账号版本时，程序会在首次启动时自动重建可恢复的目录索引；该迁移不会删除会话、下载计划、已下载文件或任何网盘内容。
...
完整参数见 [中文命令参考](docs/zh-CN/COMMANDS.md)，架构边界见 [中文架构说明](docs/zh-CN/ARCHITECTURE.md)。全部文档入口见 [docs/README.md](docs/README.md)。
...
panfetch_ai/
  ...
  core/
    ...
    plan_history.py      SQLite 下载计划历史、摘要和详情恢复
    plan_preview.py      文件夹优先的最终下载摘要
  ...
  ui/
    assistant_page.py   精简的 AI 问答页面和折叠运行详情
    download_plan_tree.py 可勾选目录树与父子状态联动
    plan_history_page.py 历史下载计划列表
...
更详细的边界说明见 [架构文档](docs/zh-CN/ARCHITECTURE.md)。
...
| `.panfetch-ai/` | SQLite 目录索引、下载计划历史与 AI 会话历史 |
| `downloads/` | 默认下载目录 |
| `exports/` | 导出的目录清单 |
| `logs/` | 脱敏运行日志 |
