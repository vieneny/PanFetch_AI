...
2. 在计划列表中选择一个历史计划，进入独立详情页。
3. 详情把同一目录的文件聚合成默认折叠、可勾选的目录树，下载器只接收选中的叶子文件。
...
凭据存放在 `.secrets/`，非敏感设置存放在 `local_settings.json`，目录索引、会话历史和下载计划历史存放在 `.panfetch-ai/`。计划列表只读取摘要，打开具体计划后才恢复规则与候选路径。这些路径均被 Git 忽略。日志会脱敏，但仍不应提交到公开仓库。
